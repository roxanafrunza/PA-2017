...
public class State {
	 ...
	 *            row position
	 * @param parent
	 *            parent state for this state
	 * @param distance
	 *            distance to the new state (should be old distance + 1)
	 */
	...
	public int approximateDistance(State other) {
		 ...
		 * finala
		 */
		d = Math.abs(this.x - other.x) * Math.abs(this.x - other.x)
				+ Math.abs(this.y - other.y) * Math.abs(this.y - other.y);
		// d = Math.abs(this.x - other.x) + Math.abs(this.y - other.y);
		return d;
	}
