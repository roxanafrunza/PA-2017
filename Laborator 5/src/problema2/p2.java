...
class Reversi {
	...
	public int eval(int player) {
		/**
		 * TODO Implementati o functie de evaluare Aceasta trebuie sa intoarca:
		 * Inf daca jocul este terminat in favoarea lui player -Inf daca jocul
		 * este terminat in defavoarea lui player
		 *
	 ...
	 * move.y == -1 semnifica ca jucatorul paseaza randul la mutare Returneaza
	 * true daca mutarea este valida
	 */
...
class P2 {
	...
	static Pair<Integer, Move> minimax(Reversi init, int player, int depth) {
		...
		/**
		 * TODO Determinati mutarile posibile si determinati cel mai bun scor si
		 * cea mai buna mutare folosind algoritmul minimax
		 *
		 * Fiti atenti ca daca player nu poate efectua nici o mutare, el poate
		 * ceda randul la mutat prin mutarea conventionala <player, -1, -1>
		 */
		ArrayList<Move> possibleMoves = new ArrayList<Move>();
		for (int i = 0; i < Reversi.N; i++)
			for (int j = 0; j < Reversi.N; j++) {
				Reversi initClone = (Reversi) init.clone();
				Move mv = new Move(player, i, j);
				if (initClone.apply_move(mv))
		...
		if (possibleMoves.isEmpty())
			return new Pair<Integer, Move>(init.eval(player), new Move(player));
		int max = -Reversi.Inf;
		Move auxMove = new Move(player);
		for (Move move : possibleMoves) {
			Reversi initNew = (Reversi) init.clone();
			initNew.apply_move(move);
			int score = -minimax(initNew, -player, depth - 1).first;
			if (score > max) {
				max = score;
				auxMove = move;
		...
		return new Pair<Integer, Move>(max, auxMove);
	}
	...
	static Pair<Integer, Move> minimax_abeta(Reversi init, int player, int depth, int alfa, int beta) {
		...
		/**
		 * TODO Determinati mutarile posibile si determinati cel mai bun scor si
		 * cea mai buna mutare folosind algoritmul minimax cu alfa-beta pruning
		 *
		 * Fiti atenti ca daca player nu poate efectua nici o mutare, el poate
		 * ceda randul la mutat prin mutarea conventionala <player, -1, -1>
		 */
		...
		Reversi initNew = (Reversi) init.clone();
		for (Move move : possibleMoves) {
			int score = -minimax_abeta(initNew, -player, depth - 1, -beta, -alfa).first;
			if (score >= alfa) {
				alfa = score;
				auxMove = move;
				init.apply_move(move);
		...
		return new Pair<Integer, Move>(alfa, auxMove);
	}
