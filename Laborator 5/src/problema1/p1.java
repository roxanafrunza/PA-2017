...
class Nim {
	...
	public boolean ended() {
		 ...
		 */
		// jocul se termina cand toate gramezile au 0 elemente
	...
	public int eval(int player) {
		 ...
		 */
		/*
		 * if (this.ended() == true) return -Inf; if (heaps[0] + heaps[1] +
		 * heaps[2] == 1) return Inf; ArrayList<Move> playerMoves =
		 * get_moves(player); return playerMoves.size();
		 */
		int sum = 0;
		for (int i = 0; i < heaps.length; i++) {
			sum += heaps[i];
		}
		//System.out.println("FUNCTIE DE EVALUARE");
		if (ended() && !winner(player))
			return -Inf;
		if (winner(player))
			return Inf;
		// ArrayList<Move> playerMoves = get_moves(player);
